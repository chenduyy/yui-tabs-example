<template>
	<view class="container">
		...
		<view class="title-wrap">
			...
			<view class="title-wrap__desc">可通过swipeAnimated开启标签内容的拖动动画，拖动动画开启后需将isLazyRender设为false</view>
		</view>
		<y-tabs pKey="8" :active="activeIndex7" animated swipeable swipeAnimated :isLazyRender="false">
			<y-tab v-for="index in 5" :key="index" :title="'标签'+index">
				<view v-for="(idx) in index*5" :key="idx">内容{{index}}</view>
			</y-tab>
		</y-tabs>
		...
		<y-tabs pKey="12" class="y-tabs" :active="activeIndex10" animated scrollThreshold="4">
			<template #title0>
				我是自定义标题
			</template>
			<y-tab v-for="index in 5" :key="index" :title="'标签'+index">
